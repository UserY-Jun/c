...
class AdminController extends BaseController{
    ...
    public function AdminAdd(){
        ...
        if(IS_POST){ 
         ...
         //  var_dump($a);exit;
            if($model->create()){
                $model->shop = cookie('admin')['shop'];
                $model->barcode = rand(100000, 999999);
                $model->add();
...
/****************/
    ...
    public function  clock(){
        //$R=I('get.r');
        $CountDate=I('CountDate');//当前点击的年.月份---------------------------------------------------------
     ...
     //        var_dump($ym);
        $model=D('clock');
        $a=$model->where(array('s_time_'=>array('EQ',$ym)))->select();
     //   var_dump($a);
        
     //   $sql = 'select * from cms_clock where 1=1 AND MONTH(s_time_)=MONTH(CURDATE()) ORDER BY id DESC;';
     // /  $oldOrder = $model->query($sql); 
        $data_=$model->search();
        
        $oldOrder = $data_['oldOrder'];
   ...
   //     echo '<hr>';
    ...
    public function EditClock($WorkType,$WorkDate,$UserID,$Remark){
        $model=D('clock');   //$WorkType = status 
        $year=  substr($WorkDate,0,4);
        $month=  (int)substr($WorkDate,5,2);
        $day=  substr($WorkDate,8);
        $where['user_id']=$UserID;
        $where['year']=$year;
        $where['month']=$month;
        $where['day']=$day;
        $ColockList=$model->field('id')->where($where)->find();
        $WorkDateTime=strtotime($WorkDate);
        $TsTime=strtotime(date('Y-m-d'));
        
        if($WorkDateTime>$TsTime){
            echo  '超过当前日期';  
        }else{
              if(empty($ColockList)){
                     $data['user_id']=$UserID;
