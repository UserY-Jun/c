...
class OrdersModel extends Model{
    ...
    {       $tid_s='tid_s';
        ...
        $page->setConfig('header','<li class="rows">共<b>%TOTAL_ROW%</b>条记录&nbsp;&nbsp;第<b>%NOW_PAGE%</b>页/共<b>%TOTAL_PAGE%</b>页</li>');
         ...
         $r=$page->firstRow; 
         $iid=cookie(order_where_id);
    
         $last=  $this->field('id')->where("")->order('id desc')->limit($r,1)->select();
         $row=  $this->field('id')->where("")->order('id desc')->limit($r,50)->select();
         $lastid=$last[0]['id']-$pagelist;
         $rowid=$row[0]['id']+$pagelist;
         //获取显示数据
         var_dump($r);
         $show=$page->show();
        /**********  取出数据 ****************/
              $data=$this
                     //     ->alias('')           //取别名  
                        ...
                        ->where("id>=($lastid)  AND id<=($rowid) ")
                    //   ->where("id<=(SELECT id FROM CMS_ORDERS LIMIT '.($page->firstRow).'1) AND id<=(SELECT id FROM CMS_ORDERS LIMIT '.($page->firstRow).+200')")
                  //     ->where($map)
                         ->group($tid_s)
                         ->order('id desc') 
                         ->limit('',$page->listRows)
